...
if (is_archive() && is_active_sidebar('sidebar')) {
    $organium_sidebar_position = organium_get_theme_mod('archive_sidebar_position');
} elseif (is_active_sidebar('sidebar')) {
    $organium_sidebar_position = organium_get_theme_mod('sidebar_position');
} else {
                    ...
                    <div class="col-lg-<?php echo ((is_active_sidebar('sidebar') && $organium_sidebar_position !== 'none') ? '9' : '12'); ?>">
                        <div class="organium_content_wrapper">
                            ...
                            <div class="organium_archive_listing">
                                <div class="organium_archive_listing_wrapper<?php echo ( !empty(get_queried_object() ) && get_queried_object()->name == 'organium-portfolio' ? ' organium_isotope_trigger grid_listing columns_3' : '' ); ?><?php echo ( !empty(get_queried_object() ) && get_queried_object()->name == 'organium-recipes' ? ' recipes_style_standard' : '' ); ?>">
                                    <?php
                                    while (have_posts()) : the_post();
                                        if ( !empty(get_queried_object() ) && get_queried_object()->name == 'organium-recipes' ) {
