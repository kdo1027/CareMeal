<div id="qlwapp" class="qlwapp-free  <?php printf("qlwapp-%s qlwapp-%s qlwapp-%s qlwapp-%s", esc_attr($button['layout']), esc_attr($button['position']), esc_attr($display['devices']), esc_attr($button['rounded'] === 'yes' ? 'rounded' : 'square')); ?>">
  <div class="qlwapp-container">
    ...
    <?php endif; ?>
   <?php $href = 'https://api.whatsapp.com/send?phone='.qlwapp_format_phone($button['phone']).'&text='.esc_html($button['message']).'' ; ?>
    <a class="qlwapp-toggle" 
       data-action="<?php echo ($button['box'] === 'yes' ? 'box' : 'open'); ?>" 
       data-phone="<?php echo qlwapp_format_phone($button['phone']); ?>" 
       data-message="<?php echo esc_html($button['message']); ?>" href="<?php echo esc_url_raw($href); ?>" target="_blank">
      <?php if ($button['icon']): ?>
