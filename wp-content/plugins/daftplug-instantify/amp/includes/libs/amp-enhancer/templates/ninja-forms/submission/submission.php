...
class AMP_Enhancer_NF_AJAX_Controllers_Submission extends AMP_Enhancer_NF_Abstracts_Controller
{
    ...
    protected function process()
    {
        ...
        */
        ...
        foreach( $form_fields as $key => $field ){
            ...
            $this->_form_cache[ 'fields' ][ $key ][ 'settings' ][ 'value' ] = isset($this->_form_data[ 'fields' ][ $field_id ][ 'value' ])? $this->_form_data[ 'fields' ][ $field_id ][ 'value' ] : '';

            // Duplicate the Field ID for access as a setting.
            $field[ 'settings' ][ 'id' ] = $field[ 'id' ];

            // Combine with submitted data.
            $field = array_merge( $field, isset($this->_form_data[ 'fields' ][ $field_id ])? $this->_form_data[ 'fields' ][ $field_id ] : array() );

            // Flatten the field array.
            if(is_array($field) && isset($field[ 'settings' ])){
...
//validation function
    protected function amp_enhancer_nf_validate_check( $field_settings, $form_data ){

        $errors = array();
        // Required check.
        if( is_array( $field_settings[ 'value' ] ) ){
            $field_settings[ 'value' ] = implode( '', $field_settings[ 'value' ] );
        }
        if( isset( $field_settings['required'] ) && 1 == $field_settings['required'] && empty( trim( $field_settings['value'] ) ) ){
            $errors[] = 'Field is required.';
        }
        if(empty($errors)){
            if($field_settings['type'] == 'spam'){
                if($field_settings['value'] != $field_settings['spam_answer'] ){
                    $errors[] = 'Incorrect Answer';
                }
            }
            if(isset($_REQUEST['confirm_field']) && !empty($_REQUEST['confirm_field'])){
                if(in_array($_REQUEST['confirm_field'], $field_settings)){
                    $field_to_confirm = $this->_form_data['fields'][$_REQUEST['confirm_field_id']]['value'];
                    if($field_to_confirm != $field_settings['value']){
                        $errors[] = 'These fields must match!';
                    }    
                }
            }
        }
    ...
    { 
        ...
        //print_r($field_class);
        if( $errors = $this->amp_enhancer_nf_validate_check( $field_settings, $this->_form_data ) ){
            $field_id = $field_settings[ 'id' ];
             $name = $field_settings['settings'][ 'label' ];
             $errorsArray = array();
             foreach($errors as $key => $message){
                 $errorsArray[$key] = $name." ".$message;
            }

            $this->_errors[ 'fields' ][ $field_id ] = $errorsArray;
            $this->_respond();
