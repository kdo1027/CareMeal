...
class AMP_Enhancer_WPCF7_ContactForm {
	...
	public function form_html( $args = '' ) {
		...
		if(class_exists('WPCF7')){
		    $site_keys = WPCF7::get_option( 'recaptcha' );
		    if(isset($site_keys) && !empty($site_keys)){
			    $sitekeys = array_keys( $site_keys );
			    if(isset($sitekeys[0])){
			    	$sitekey = $sitekeys[0];
				}

			    $html .= '<amp-recaptcha-input layout="nodisplay" name="_wpcf7_recaptcha_response" data-sitekey="'.esc_attr($sitekey).'" data-action="_wpcf7_recaptcha_response"></amp-recaptcha-input>';
		    }
		 }
