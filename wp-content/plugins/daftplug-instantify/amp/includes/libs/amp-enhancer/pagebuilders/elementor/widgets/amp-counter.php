...
class AMP_Counter extends Widget_Base {
	...
	protected function render() {
		...
		if ( ! empty( $settings['thousand_separator'] ) ) {
			...
			$ending_number = number_format($ending_number,0,".",$delimiter);
		}
		...
		<div class="elementor-counter">
			<div class="elementor-counter-number-wrapper">
				...
				<span <?php echo $this->get_render_attribute_string( 'counter' ); ?>><?php echo esc_html($ending_number); ?></span>
				<span class="elementor-counter-number-suffix"><?php echo $settings['suffix']; ?></span>
			</div>
			<?php if ( $settings['title'] ) : ?>
				<div <?php echo $this->get_render_attribute_string( 'counter-title' ); ?>><?php echo esc_html($settings['title']); ?></div>
			<?php endif; ?>
		</div>
		<?php
	}
