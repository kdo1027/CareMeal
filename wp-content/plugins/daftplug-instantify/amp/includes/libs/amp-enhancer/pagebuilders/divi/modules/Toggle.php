<?php
if(!class_exists('AMP_Enhancer_ET_Builder_Toggle_Module')){
class AMP_Enhancer_ET_Builder_Toggle_Module extends ET_Builder_Module {
	function init() {
	...
	public function render( $attrs, $content, $render_slug ) {
		...
		$this->generate_styles(
		 ...
		 $toggelContent = '<div%4$s class="%2$s">
				%6$s
				%5$s
				%1$s
		...
		if( 'et_pb_toggle' === $render_slug ){
			...
			$toggelContent = '<amp-accordion disable-session-states><section%4$s class="%2$s et_pb_tgl">
				%6$s
				%5$s
				%1$s
				<div class="et_pb_toggle_content clearfix">
			...
			</section></amp-accordion><!-- .et_pb_toggle -->';
		}
...
$toggle_obj = new AMP_Enhancer_ET_Builder_Toggle_Module();
remove_shortcode( 'et_pb_toggle' );
add_shortcode( 'et_pb_toggle', array($toggle_obj , '_render') );
}