<?php
if(!class_exists('AMP_Enhancer_ET_Builder_Accordion_Module')){
class AMP_Enhancer_ET_Builder_Accordion_Module extends ET_Builder_Module {
	function init() {
	...
	public function render( $attrs, $content, $render_slug ) {
		...
		$a = $xpath->query( "//div[contains(@class, 'et_pb_toggle et_pb_module')]" );
		foreach($a as $key=>$currentTabNode){
			$newNode = $tmpDoc->createElement('section');
			if($key==0){
				$newNode->setAttribute('expanded', '');
			}
			foreach ($currentTabNode->attributes as $attribute){
				$newNode->setAttribute($attribute->name, $attribute->value);
				$currentTabNode->removeAttribute($attribute->name);
			}
			$aCopy = array();
			foreach($currentTabNode->childNodes as $childNodes){
				$aCopy[] = $childNodes;
			}
			foreach($aCopy as $newChild){
				$newNode->appendChild($newChild);
			}
			$tmpDoc->appendChild($newNode);
		}
		$content = $tmpDoc->saveHTML();
		$content = preg_replace("/<!DOCTYPE(.*?)<\/html>/s", "",$content);
		

		$output = sprintf(
			'<amp-accordion disable-session-states expand-single-section %3$s class="%2$s">
				%5$s
				%4$s
				%1$s
			</amp-accordion> <!-- .et_pb_accordion -->',
			$content,
...
$accordion_obj = new AMP_Enhancer_ET_Builder_Accordion_Module();//et_pb_accordion_item
remove_shortcode( 'et_pb_accordion' );
add_shortcode( 'et_pb_accordion', array($accordion_obj, '_render'));
}