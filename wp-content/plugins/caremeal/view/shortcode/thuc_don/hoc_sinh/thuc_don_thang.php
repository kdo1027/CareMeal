<?php
function create_shortcode_thuc_don_thang() {
	ob_start();
	global $wpdb;
	$user=wp_get_current_user();
	$user_id = $user->ID;
	$list_user=$wpdb->get_results( 'SELECT * FROM '.$wpdb->prefix.'users WHERE ID != ""');
	//$user_id=get_user_meta( $user_id, 'user_registration_input_box_1606780902', true);
			...
			<div id="tuan_1_ct" class="active">
				...
				<div class="cm_tab_content_tuan">
					...
					for($i=1;$i<8;$i++){
						...
						$phuhuynh = $wpdb->get_row( 'SELECT * FROM '.$wpdb->prefix.'cm_thong_ke WHERE ngay = "'.$vb_ngay_check.'" AND user_id ="'.$user_id.'"' );
						$phuhuynh_sang =  $wpdb->get_row( 'SELECT * FROM '.$wpdb->prefix.'cm_thong_ke WHERE ngay = "'.$vb_ngay_check.'" AND an_sang !="" AND user_id ="'.$user_id.'"' );
						$phuhuynh_trua = $wpdb->get_row( 'SELECT * FROM '.$wpdb->prefix.'cm_thong_ke WHERE ngay = "'.$vb_ngay_check.'" AND an_trua !=""' );
						$phuhuynh_toi = $wpdb->get_row( 'SELECT * FROM '.$wpdb->prefix.'cm_thong_ke WHERE ngay = "'.$vb_ngay_check.'" AND an_toi !="" AND user_id ="'.$user_id.'"' );
						$phuhuynh_phutoi = $wpdb->get_row( 'SELECT * FROM '.$wpdb->prefix.'cm_thong_ke WHERE ngay = "'.$vb_ngay_check.'" AND phu_toi !="" AND user_id ="'.$user_id.'"' );
						if($cm_check_ngay){
											...
											if($phuhuynh && $phuhuynh_toi){
												...
												<table>
													...
													<tbody>
														...
														$mon_an_sh=[];
														for($j=0;$j<count($mon_an_day)-1; $j++){
															$mon_an_sh[$i] = $wpdb->get_row( 'SELECT * FROM '.$wpdb->prefix.'cm_mon_an WHERE ten = "'.$mon_an_day[$j].'"');
															echo '<tr><td style="width:6%"><img src="'.$mon_an_sh[$j]->anh_av.'"></td><td style="width:20%">'.$mon_an_sh[$j]->ten.'</td><td style="width:14.5%">'.$mon_an_sh[$j]->nang_luong.'</td><td style="width:8.5%">'.$mon_an_sh[$j]->protein.'</td><td style="width:8.5%">'.$mon_an_sh[$j]->glucid.'</td><td style="width:8.5%">'.$mon_an_sh[$j]->lipid.'</td>
															<td style="width:8.5%">'.$mon_an_sh[$j]->canxi.'</td>
															<td style="width:8.5%">'.$mon_an_sh[$j]->sat.'</td>
															<td style="width:8.5%">'.$mon_an_sh[$j]->kem.'</td>
...
add_shortcode( 'thu_don_thang', 'create_shortcode_thuc_don_thang' );